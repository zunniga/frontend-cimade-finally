...
const StudentForm: React.FC<StudentFormProps> = ({
  ...
  const handleFetchError = (error: any) => {
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      setErrorMessage(
        "Error al cargar datos del estudiante:" + error.response.data
      );
    } else {
  ...
  const onSubmit: SubmitHandler<StudentFormData> = async (data) => {
    try {
      ...
      if (
        !isNum(data.code) ||
        (data.code.length !== 8 && data.code.length !== 9)
      ) {
        setError("code", {
          type: "manual",
          message:
            "El codigo debe contener solo números y exactamente 9 digitos",
        });
        return;
      }
      /* if (!isNum(data.hour)) {
  ...
  return (
    <Modal open={dataLoaded} onClose={onCloseModal}>
      <div className="max-w-screen-lg mx-auto border p-4 rounded-xl">
        <h1 className="text-md font-bold bg-[#006eb0] text-gray-100 border p-2 rounded-lg mb-4 uppercase">
          Actualizar estudiante
        </h1>
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="grid grid-cols-2 uppercase md:grid-cols-2 gap-2"
        >
          <div className="flex justify-between col-span-2 whitespace-pre-wrap">
            <div className="mb-4">
              ...
              <input
                ...
                className={`border bg-white rounded-lg p-1.5 lg:w-36 w-24 ${
                  errors?.documentNumber ? " border-red-500" : ""
                }`}
              />
              {errors?.documentNumber && (
                <span className="text-left text-xs font-mono block text-red-400">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className={`border bg-white rounded-lg p-1.5 lg:w-32 w-28 ${
                  errors?.code ? "border-red-500" : ""
                }`}
              />
              {errors?.code && (
                <span className="text-left lg:ml-4 text-xs font-mono block text-red-400">
                  {errors.code.message}
                </span>
              )}
            </div>
          ...
          <div className="flex justify-between col-span-2 whitespace-pre-wrap">
            <div className="mb-4">
              ...
              <input
                {...register("hour")}
                className={`border bg-white rounded-lg p-2 lg:w-32 w-24 ${
            ...
            <div className="mb-4">
              ...
              <input
                {...register("date")}
                className="border bg-white rounded-lg p-2 lg:w-32 w-28"
              />
            </div>
          </div>
