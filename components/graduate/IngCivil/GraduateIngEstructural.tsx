...
import { modulesIngEstructural } from "@/components/utils/courses/course";
...
const [imageUrl, setImageUrl] = useState('');
  ...
  const handleClick = () => {
    const newWindow = window.open('/brochuress/BROCHURE-INGENIERÍA ESTRUCTURAL.pdf');
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement('a');
    link.href = '/brochuress/BROCHURE-INGENIERÍA ESTRUCTURAL.pdf';
    link.download = 'BROCHURE-INGENIERÍA ESTRUCTURAL.pdf';
    link.click();
  };
            ...
            {isAccordionOpen1 && (
              <motion.div
                ...
                <div className="text-sm font-sans font-semibold">
                  <ul className="">
                  {modulesIngEstructural.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
