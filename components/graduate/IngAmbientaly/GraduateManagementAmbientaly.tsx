...
import { modulesManagementAmbientaly } from "@/components/utils/courses/course";
import { BiChevronDown, BiChevronUp } from "react-icons/bi";
import { FaRegFilePdf, FaWhatsapp } from "react-icons/fa6";
import Modal  from "@/components/share/Modal";
import Image from "next/image";

const GraduateManagementAmbientaly = () => {
...
const [imageUrl, setImageUrl] = useState('');
  ...
  const handleClick = () => {
    const newWindow = window.open('/BROCHURE/GESTION_AMBIENTAL_MUNICPAL_REGIONAL.pdf');
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement('a');
    link.href = '/BROCHURE/GESTION_AMBIENTAL_MUNICPAL_REGIONAL.pdf';
    link.download = 'GESTION_AMBIENTAL_MUNICPAL_REGIONAL.pdf';
    link.click();
  };
       ...
       <div className="md:px-20 flex flex-col sm:flex-row justify-center">
          <div className="sm:w-96 mb-4 sm:mb-0">
            <Image
              ...
              src="/graduate/impacto_ambiental.webp"
              alt="icon"
              width={800}
              height={800}
              priority={true}
          ...
          <div className="sm:w-1/2 md:px-20 sm:px-5 lg:mt-4">
            <div className="text-pink-600 font-extrabold font-poppins lg:text-3xl text-2xl mb-4">
              GESTI&Oacute;N AMBIENTAL MUNICPAL Y REGIONAL
            </div>
            <div className="mb-4">
              <p className="text-sm font-semibold text-gray-600 italic">
                Diplomado dirigido a
                gerentes municipales, gerentes de gestión ambiental, sub gerentes de 
                gestión ambiental, sub gerentes de medio ambientes de las 
                municipalidades provinciales y distritales, jefes de áreas en gestión 
                ambiental, jefes de área de residuos sólidos, asistentes administrativos, 
                practicantes, funcionarios de las municipalidades, profesionales interesados 
                en la materia, y estudiantes universitarios.
              </p>
            </div>
            <div className="grid lg:grid-cols-2 grid-cols-1 text-pink-500 text-sm mb-4">
              ...
              <Image
                ...
                height={20}
                priority={true}/>
                <p className="font-bold ml-1">Inicio 14 de Abril</p>
              </div>
              ...
              <div className="grid lg:grid-cols-3 md:grid-cols-1 grid-cols-1 gap-2">
                <div className="w-full">
                  <Link
                    ...
                    href='https://wa.me/51984040264?text=Hola,%20deseo%20más%20información%20sobre%20el%20diplomado%20de%20Gestión%20Ambiental%20Municipal%20y%20Regional'
                    target="_blank">
            ...
            {isAccordionOpen1 && (
              <motion.div
                ...
                <div className="text-sm font-sans font-semibold">
                  <ul className="">
                  {modulesManagementAmbientaly.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
...
export default GraduateManagementAmbientaly;
