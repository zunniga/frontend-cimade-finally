...
import { modulesQualityMonitoringAmbientaly } from "@/components/utils/courses/course";
import { BiChevronDown, BiChevronUp } from "react-icons/bi";
import { FaRegFilePdf, FaWhatsapp } from "react-icons/fa6";
import Modal  from "@/components/share/Modal";
import Image from "next/image";

const GraduateQualityMonitoringAmbientaly = () => {
...
const [imageUrl, setImageUrl] = useState('');
  ...
  const handleClick = () => {
    const newWindow = window.open('/BROCHURE/MONITOREO_EVALUACION_CALIDAD AMBIENTAL.pdf');
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement('a');
    link.href = '/BROCHURE/MONITOREO_EVALUACION_CALIDAD AMBIENTAL.pdf';
    link.download = 'MONITOREO_EVALUACION_CALIDAD AMBIENTAL.pdf';
    link.click();
  };
       ...
       <div className="md:px-20 flex flex-col sm:flex-row justify-center">
          ...
          <div className="sm:w-1/2 md:px-20 sm:px-5 lg:mt-4">
            <div className="text-pink-600 font-extrabold font-poppins lg:text-3xl text-2xl mb-4">
              MONITOREO Y EVALUACI&Oacute;N DE LA CALIDAD AMBIENTAL
            </div>
            <div className="mb-4">
              <p className="text-sm font-semibold text-gray-600 italic">
                Diplomado dirigida a bachilleres, titulados y profesionales de las carreras de ingenierías 
                ambiental, químicos, agrónomo, sanitarios, biólogos y ramas afines 
                que se encuentran involucrados en las actividades ambientales en 
                empresas mineras o instituciones públicas como MINAM, MEM, OEFA y 
                SERNANP.
              </p>
            </div>
              ...
              <div className="grid lg:grid-cols-3 md:grid-cols-1 grid-cols-1 gap-2">
                <div className="w-full">
                  <Link
                    ...
                    href='https://wa.me/51984040264?text=Hola,%20deseo%20más%20información%20sobre%20el%20diplomado%20de%20Monitoreo%20y%20Evaluación%20de%20la%20Calidad%20Ambiental'
                    target="_blank">
            ...
            {isAccordionOpen1 && (
              <motion.div
                ...
                <div className="text-sm font-sans font-semibold">
                  <ul className="">
                  {modulesQualityMonitoringAmbientaly.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
...
export default GraduateQualityMonitoringAmbientaly;
